...
class GrovsServiceBatchTest {
    ...
    fun setUp() {
        ...
        E2ETestUtils.setupMockUserAgent("Grovs SDK batch tests")
        server = MockWebServer().apply { start() }
        val grovsContext = GrovsContext().also { it.settings.baseURL = server.url("/").toString() }
    ...
    fun `system events are posted to events batch as an events array`() = runTest {
        server.enqueue(MockResponse().setResponseCode(200).setBody("""{"accepted":2,"rejected":0,"errors":[]}"""))
    ...
    fun `custom events are posted to the same endpoint with event_name`() = runTest {
        ...
        val body = JSONObject(server.takeRequest().body.readUtf8()).getJSONArray("events")
    ...
    fun `a 4xx is Error`() = runTest {
        server.enqueue(MockResponse().setResponseCode(400).setBody("""{"error":"events must be an array"}"""))
